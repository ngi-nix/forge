...
"""Generate a JSON list of package link failures for NGI Forge recipes.

By default, this script automatically:
  1. Generates forge.csv using maintainers/forge-meta.nix
  2. Runs lychee link checker (--format json)
  3. Maps failing URLs to package names and outputs structured JSON

USAGE:
  # Automatically generate forge.csv, check links, and emit JSON report:
  ./maintainers/forge-meta-failures.py | tee failures.json

  # Or use pre-generated forge.csv and lychee.json:
...
def parse_csv(csv_path: str) -> dict[int, dict[str, str]]:
    """Parse forge.csv into a mapping from 1-indexed line numbers to package metadata."""
    pkgs = {}
...
def run_checks() -> tuple[dict[int, dict[str, str]], str]:
    """Automatically generate forge.csv and run lychee link checker."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    repo_root = os.path.dirname(script_dir)
    nix_file = os.path.join(script_dir, "forge-meta.nix")

    with tempfile.TemporaryDirectory() as tmpdir:
        csv_path = os.path.join(tmpdir, "forge.csv")

        print(
            "Generating forge.csv from maintainers/forge-meta.nix...",
            file=sys.stderr,
        )
        cmd_csv = [
            "nix",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate JSON list of attribute sets mapping NGI Forge package names to link failures."
    )
    parser.add_argument(
        "csv_path",
        nargs="?",
        default=None,
        help="Optional path to forge.csv (if omitted, automatically generated via nix eval)",
    )
    parser.add_argument(
        "json_path",
        nargs="?",
