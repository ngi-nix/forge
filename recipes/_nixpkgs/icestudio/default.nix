...
buildNpmPackage (finalAttrs: {
  ...
  installPhase = ''
    ...
    makeWrapper ${finalAttrs.passthru.nwjs}/bin/nw $out/bin/icestudio \
        --add-flags $out \
  ...
  passthru = {
    ...
    # Current nwjs version (0.102.1) is insecure:
    # Uses Chrome 139.0.7258.128, known to have many vulnerabilities.
    #
    # Attempt to update: https://github.com/NixOS/nixpkgs/pull/462257
    nwjs = nwjs.overrideAttrs (
      finalAttrs': oldAttrs': {
        version = "0.111.1";
        src =
          let
