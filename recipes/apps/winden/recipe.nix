...

{
  apps.winden = {
    ...
    longDescription = ''
      ...
      - It is identity-free, meaning that senders and receivers don't need to know each other's identity to use it, or to reveal their identity to winden.
      - It does not require people to sign up or log in and also cannot access any files you send, as they are end-to-end encrypted.
      - Files are never stored on winden's servers and transfers happen in real-time.
      - While these aspects ensure the app is more private and secure, it means that both the sender and receiver need to be online at the same time.
      - Is based on the [Magic Wormhole protocol](https://magic-wormhole.readthedocs.io/), Winden was developed to scale the protocol without compromising its security and make it ready for web-usage.

      **Learn more about how Winden works in the [FAQ](https://winden.app/faq)**.
    '';
    usage = ''
      > [!NOTE]
      ...
      - Run the app in a container or nixos vm
      - Visit http://localhost:8080 in two tabs in your browser to send and receive files.
    '';
