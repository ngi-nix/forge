...
in
{
  apps.pixelfed = {
    ...
    usage = ''
      ...
      **Containers:**
      ```bash
      podman exec -it $(podman ps -qf name=pixelfed_web) pixelfed-manage key:generate --show
      ```
      ...
      **Containers:**
      ```bash
      podman exec -it $(podman ps -qf name=pixelfed_web) pixelfed-manage user:create \
        --name="Main User" \
        --username="main" \
    ...
    services = {
      ...
      components.worker = {
        process = {
          command = pkgs.writeShellScriptBin "start-pixelfed-worker" ''
            cd /var/lib/pixelfed
