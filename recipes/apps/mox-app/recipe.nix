...

{
  ...
  services = {
    components.mox = {
      ...
      '';
      command = systemConfig.packages.mox;
      argv = [
        "-config"
    ...
    runtimes = {
      container = {
        ...
        setup = ''
          ...
          if ! [ -d /var/lib/mox ]; then
            ...
            # Generate DKIM keys
            mkdir -p config/dkim
            ${lib.getExe systemConfig.packages.mox} dkim genrsa > config/dkim/dkima.rsa2048.privatekey.pkcs8.pem
            ${lib.getExe systemConfig.packages.mox} dkim genrsa > config/dkim/dkimb.rsa2048.privatekey.pkcs8.pem
        ...
        packages = [
          ...
          systemConfig.packages.mox # required for admin tasks
      ...
      nixos = {
        ...
        '';
        packages = [ systemConfig.packages.mox ];
        extraConfig = {
